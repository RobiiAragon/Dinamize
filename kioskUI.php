...
<?php endif; ?>
    <div class="dashboard-container">
        ...
        <main class="main-content">
          ...
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 500" preserveAspectRatio="xMidYMin meet">            <g id="piso">
            ...
            <g id="local11" class="local-clickable" style="cursor: pointer">
            <rect class="cls-1" x="377.56" y="647.47" width="83.91" height="71.74"/>
                <?php
                $conn = OpenCon();
                $sql = "SELECT n.* FROM negocios n WHERE n.NumeroLocal = 11";
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    $negocio = $result->fetch_assoc();
                    ?>
                    <foreignObject 
                      x="<?= $coordenadasLocales[11]['x'] ?>" 
                      y="<?= $coordenadasLocales[11]['y'] ?>" 
                      width="<?= $coordenadasLocales[11]['width'] ?>" 
                      height="<?= $coordenadasLocales[11]['height'] ?>">
                      <div xmlns="http://www.w3.org/1999/xhtml" class="local">
                          <img src="data:image/jpeg;base64,<?= base64_encode($negocio['logo']) ?>" 
                              alt="Logo de <?= htmlspecialchars($negocio['nombre']) ?>"
                              class="local-logo">
            ...
            <g id="local12" class="local-clickable" style="cursor: pointer">
            <polygon class="cls-1" points="85.13 203.21 132.11 252.57 189.05 196.32 140.79 149.34 85.13 203.21"/>
                <?php
                $conn = OpenCon();
                $sql = "SELECT n.* FROM negocios n WHERE n.NumeroLocal = 12";
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    $negocio = $result->fetch_assoc();
                    ?>
                    <foreignObject 
                      x="<?= $coordenadasLocales[12]['x'] ?>" 
                      y="<?= $coordenadasLocales[12]['y'] ?>" 
                      width="<?= $coordenadasLocales[12]['width'] ?>" 
                      height="<?= $coordenadasLocales[12]['height'] ?>">
                      <div xmlns="http://www.w3.org/1999/xhtml" class="local">
                          <img src="data:image/jpeg;base64,<?= base64_encode($negocio['logo']) ?>" 
                              alt="Logo de <?= htmlspecialchars($negocio['nombre']) ?>"
                              class="local-logo">
            ...
            <g id="local13" class="local-clickable" style="cursor: pointer">
            <polygon class="cls-1" points="132.11 252.57 195.43 318.62 258.75 254.28 195.43 191.47 132.11 252.57"/>
            <?php
                $conn = OpenCon();
                $sql = "SELECT n.* FROM negocios n WHERE n.NumeroLocal = 13";
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    $negocio = $result->fetch_assoc();
                    ?>
                    <foreignObject 
                      x="<?= $coordenadasLocales[13]['x'] ?>" 
                      y="<?= $coordenadasLocales[13]['y'] ?>" 
                      width="<?= $coordenadasLocales[13]['width'] ?>" 
                      height="<?= $coordenadasLocales[13]['height'] ?>">
                      <div xmlns="http://www.w3.org/1999/xhtml" class="local">
                          <img src="data:image/jpeg;base64,<?= base64_encode($negocio['logo']) ?>" 
                              alt="Logo de <?= htmlspecialchars($negocio['nombre']) ?>"
                              class="local-logo">
